...
public:
    ...
    void
    start(std::function<void(std::string &device_id, std::string &username, std::vector<uint8_t> &payload)> callback);
...
private:
    ...
    void
    listen(std::function<void(std::string &device_id, std::string &username, std::vector<uint8_t> &payload)> &callback);
