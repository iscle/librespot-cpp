/*
 * Diffie-Hellman 128bit Key Exchange
 * Adapted from https://github.com/thejinchao/dhexchange
 */
...
static void inline _u128_make(uint128_t *dq, const DH_KEY key) {
    memcpy(dq->byte, key, DH_KEY_LENGTH);
}

static int inline _u128_is_zero(const uint128_t *dq) {
    return dq->low == 0 && dq->high == 0;
}

static int inline _u128_is_odd(const uint128_t *dq) {
    return dq->low & 1;
}

static void inline _u128_lshift(uint128_t *dq) {
    uint64_t t = (dq->low >> 63) & 1;
    dq->high = (dq->high << 1) | t;
    dq->low = dq->low << 1;
}

static void inline _u128_rshift(uint128_t *dq) {
    uint64_t t = (dq->high & 1) << 63;
    dq->high = dq->high >> 1;
    dq->low = (dq->low >> 1) | t;
}

static int inline _u128_compare(const uint128_t *a, const uint128_t *b) {
    if (a->high > b->high) {
        return 1;
...
static void inline _u128_add(uint128_t *r, const uint128_t *a, const uint128_t *b) {
    uint64_t overflow = 0;
...
static void inline _u128_add_i(uint128_t *r, const uint128_t *a, const uint64_t b) {
    uint64_t overflow = 0;
...
static void inline _u128_sub(uint128_t *r, const uint128_t *a, const uint128_t *b) {
    uint128_t invert_b;
...
/* r = a*b mod P */
static void _mulmodp(uint128_t *r, uint128_t a, uint128_t b) {
    uint128_t t;
    uint128_t double_a;
    uint128_t P_a;
...
/* r = a^b mod P */
static void _powmodp(uint128_t *r, uint128_t a, uint128_t b) {
